...
const int SumOfDestination = 173;
...
private:
	...
	double y;
public:
	int Nearest (const Coordinate Station[StationNumber_Max]);
};

int Coordinate::Nearest(const Coordinate Station[StationNumber_Max]){
	int temp;
...
int main()
{
	...
	while(CorrectInput4 == false){
		...
		if (FourthChoose > temp_Number_Max)
			cout << "此選項不存在";
		else CorrectInput4 = true;
	}
	//cout << place[FirstChoose][SecondChoose].Nearest(const Coordinate Station[StationNumber_Max]);
	//cout << place[ThirdChoose][FourthChoose].Nearest(const Coordinate Station[StationNumber_Max]);
	return 0;
}
